...
# Remove a TOML section: from section header (matching section_pattern) through its key=value lines,
# until (and excluding) the next line that starts with [. That next section header line is kept.
# Uses sed range so we don't corrupt the rest of the file (unlike the original awk which could).
remove_toml_section() {
    ...
    if grep -q "$section_pattern" "$pyproject_file"; then
        ...
        sed -e '/'"$section_pattern"'/,/^\[/ { /'"$section_pattern"'/d; /^\[/!d; }' "$pyproject_file" > "$pyproject_file.tmp" && mv "$pyproject_file.tmp" "$pyproject_file"
...
remove_toml_section "$AGENTS_PYPROJECT" '^\[tool\.uv\.sources\]' '[tool.uv.sources]'
remove_toml_section "$AGENTS_PYPROJECT" '^\[project\.entry-points."uipath\.middlewares"\]' '[project.entry-points."uipath.middlewares"]'
...
remove_toml_section "$LANGCHAIN_PYPROJECT" '^\[tool\.uv\.sources\]' '[tool.uv.sources]'
...
echo "  - uipath-agents-python: [tool.uv.sources] and [project.entry-points.\"uipath.middlewares\"] removed"
