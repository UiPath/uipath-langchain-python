...
class AgentEvaluationResults(BaseModel):
    """Complete evaluation results for an agent across all test cases."""

    results: List[EvaluationRunResult] = Field(
        description="List of all evaluation result entries"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional metadata about the evaluation run"
    )
...
class AgentBaseClass(BaseModel):
    ...
    def end_execution_tool(self) -> EndExecutionTool:
        return EndExecutionTool(
            args_schema=self.output_schema, tool_output_schema=self.output_schema
        )
...
async def chatbot_node(
    ...
    if (
        ...
        and "claude" in getattr(llm, "model_name", "")
    ):
...
def should_continue(
    state: StateBaseClass,
) -> Literal["tools", "raise_error", "end_execution"]:
...
class BasicLoop:
    def __init__(
        ...
        self.llm_with_tools = llm.bind_tools(
            list[BaseTool](scenario.tools)
            + [scenario.end_execution_tool, scenario.raise_error_tool],
    ...
    def prepare_input(self, state: BaseModel) -> StateBaseClass:
        ...
        try:
            new_state.messages = [
                ...
                HumanMessage(
                    content=self.scenario.user_prompt.format_map(state.model_dump())
                ),
            ]
            if self.debug:
                print("[DEBUG] Created messages from input")
                print(
                    f"[DEBUG] Messages: {[m.content[:50] for m in new_state.messages]}"
                )
        except KeyError:
    ...
    def build_graph(self) -> StateGraph[Any, Any]:
