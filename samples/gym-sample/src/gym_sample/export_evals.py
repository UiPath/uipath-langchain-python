...
def evaluator_to_spec(evaluator: BaseEvaluator[Any, Any, Any], agent_name: str) -> Dict[str, Any]:
...
def datapoint_to_evaluation(
    ...
    for evaluator_name, criteria in datapoint.evaluation_criterias.items():
    ...
    return {
        ...
        "expectedAgentBehavior": datapoint.expected_agent_behavior,
        "simulateInput": False,
...
def export_evaluators(
    ...
    for evaluator_name in evaluators:
        ...
        base_name = re.sub(r"_\d+$", "", evaluator_name)

        if only_supported and base_name not in SUPPORTED_EVALUATORS:
            print(f"  ⊘ Skipped {evaluator_name} (not yet supported in PR)")
            continue
        spec = evaluator_to_spec(evaluators[evaluator_name], agent_name)
...
def export_eval_set(
    ...
    eval_set = {
        ...
        "evaluations": [
            ...
            for dp in agent.evaluation_set.evaluations
        ],
        "modelSettings": [],
        "createdAt": "2025-01-01T00:00:00.000Z",
        "updatedAt": "2025-01-01T00:00:00.000Z",
    ...
    if small_set_size > 0:
        eval_set["evaluations"] = [
            datapoint_to_evaluation(dp, eval_set_id, evaluator_refs, agent_name)
            for dp in agent.evaluation_set.evaluations[:small_set_size]
        ]
    ...
    print(
        f"  ✅ Exported eval set with {len(agent.evaluation_set.evaluations)} evaluations"
    )
...
def export_agent(
    ...
    # Put files in the agent's directory
    agent_dir = base_dir / agent_name

    # Export evaluators
    evaluators_dir = agent_dir / "evaluators"
    evaluator_ids = export_evaluators(
        agent_name, evaluators_dir, only_supported, include_llm_judge
    )

    # Export eval set
    eval_sets_dir = agent_dir / "eval-sets"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Export evaluators and eval sets for agents"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--exclude_llm_judge", action="store_true", help="Include LLM judge evaluators"
    )
    parser.add_argument(
        "--small_set_size",
        type=int,
    ...
    for agent_name in get_agents().keys():
        export_agent(
            agent_name,
            base_dir,
            only_supported=not args.include_not_supported,
            include_llm_judge=not args.exclude_llm_judge,
            small_set_size=args.small_set_size,
        )
