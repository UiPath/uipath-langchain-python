...
Message: TypeAlias = (
    BaseMessage | SystemMessage | ToolMessage | HumanMessage | AIMessage
)
...
class EndExecutionTool(StructuredTool):
    ...
    tool_output_schema: type[BaseModel] | None = (
        None  # The final output schema to return
    )
    ...
    def run(self, tool_input: StateBaseClass, *args: Any, **kwargs: Any):
        ...
        if self.tool_output_schema:
            # Convert the result dict to the output_schema type
            return self.tool_output_schema.model_validate(tool_input.result)
