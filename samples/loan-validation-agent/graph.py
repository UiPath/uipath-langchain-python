...
7. Provide a detailed validation report in the following valid JSON format:
  {{
  "validation_report": {{
   "is_valid": boolean,
   "general_checks": {{
...
class GeneralChecks(BaseModel):
    name_consistency: str
    document_recency: str
    income_verification: str
    address_consistency: str
    document_quality: str
    signature_verification: str

class StateSpecificChecks(BaseModel):
    drivers_license: Optional[str] = None
    florida_drivers_license: Optional[str] = None
    pay_stub_recency: str
    bank_statement_address: str
    flood_insurance: str
    employer_verification: str
...
def research_node(state: GraphInput) -> GraphOutput:
    ...
    try:
      ...
      if not json_match:
          raise ValueError("No JSON object found in the AI's response." + ai_message_content)
      ...
      report = Report(
          ...
          general_checks=validation_report["general_checks"],
          state_specific_checks=validation_report["state_specific_checks"],
