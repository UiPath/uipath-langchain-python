...
class llmRule(BaseModel):
    ...
    conditions: Dict = Field(
        default={},
        description="Conditions must have this form {'predicate': ['value1', 'value2']}",
    )
...
async def fetch_emails(state: GraphState) -> Command:
    ...
    try:
        ...
        return Command(update={"emails": emails})
...
async def fetch_folders(state: GraphState) -> Command:
    ...
    try:
        ...
        return Command(update={"folders": folders})
        ...
        return Command(
            update={
                "output": GraphOutput(
                    success=False,
                    rules_created=0,
                    message=f"Error fetching folders: {e}",
                )
            }
        )
...
async def fetch_rules(state: GraphState) -> Command:
    ...
    try:
        ...
        return Command(update={"rules": rules})
        ...
        return Command(
            update={
                "output": GraphOutput(
                    success=False, rules_created=0, message=f"Error fetching rules: {e}"
                )
            }
        )
...
def _infer_conditions_from_rule(llm_rule: llmRule, emails: List[Email]) -> Dict:
    ...
    if emails_mentioned:
        ...
        if "azure" in rule_text:
            ...
            for email in emails:
                if any(
                    keyword in email.sender.lower() for keyword in target_folder.split()
                ):
...
Analyze the email patterns and create the most valuable organizational rules.
        ...
        if hasattr(response, "rules"):
            ...
            rules_list = response["rules"]
            rules_list = [
                llmRule(**rule) if isinstance(rule, dict) else rule
            ...
            rules_list = [
                llmRule(**rule) if isinstance(rule, dict) else rule for rule in response
            ]
        else:
            logger.warning(
                f"Unexpected response format: {type(response)}, content: {response}"
            )
            rules_list = []
        ...
        for idx, llm_rule in enumerate(rules_list):
            if isinstance(llm_rule, dict):
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to create llmRule from dict {rule_dict}: {e}"
                    )
                    continue
        ...
        logger.info(
            f"Generated {len(suggestions)} rule suggestions using structured output"
        )

        return Command(update={"suggestions": suggestions})
...
async def wait_for_human_approval(state: GraphState) -> Command:
    ...
    for idx, suggestion in enumerate(state.suggestions, 1):
        ...
        if suggestion.rule_type == "IMPROVED":
            suggestions_text += (
                "   This will enhance an existing rule with additional conditions\n"
            )
        else:
    ...
    suggested_folders = set(
        r.target_folder for r in state.suggestions if r.rule_type == "NEW"
    )
    ...
    human_approved = (
        isinstance(action_data.get("Answer"), bool) and action_data["Answer"] is True
    )

    return Command(update={"human_approved": human_approved})
...
async def create_rules(state: GraphState) -> Command:
    ...
    try:
        ...
        for idx, suggestion in enumerate(state.suggestions, 1):
            logger.info(
                f"Processing rule {idx}/{len(state.suggestions)}: {suggestion.rule_name} ({suggestion.rule_type})"
            )
                ...
                if suggestion.rule_type == "IMPROVED":
                    ...
                    matched_rule = next(
                        (
                            ...
                            if r.rule_name == suggestion.rule_name
                        ),
                        None,
                    )
                    ...
                    if not matched_rule:
                        for r in existing_rules:
                            ...
                            cond_match = conditions_overlap(
                                r.conditions, suggestion.conditions
                            )
                            name_similarity = SequenceMatcher(
                                None, r.rule_name, suggestion.rule_name
                            ).ratio()
                    ...
                    if matched_rule:
                        ...
                        folder_id = state.folders.get(
                            suggestion.target_folder,
                            matched_rule.actions.get("moveToFolder", ""),
                        )
                        if not folder_id:
                            errors.append(
                                f"No folder ID found for improved rule {suggestion.rule_name}"
                            )
                            continue
                        try:
                            await state.outlook_client._delete(
                                f"mailFolders/inbox/messageRules/{matched_rule.id}"
                            )
                            logger.info(
                                f"Deleted existing rule: {matched_rule.rule_name}"
                            )
                        except Exception as e:
                            logger.warning(
                                f"Could not delete existing rule {matched_rule.rule_name}: {e}"
                            )
                        rule_data = {
                        ...
                        rule_id = await state.outlook_client.create_message_rule(
                            rule_data
                        )
                        if rule_id:
                            rules_updated += 1
                            ...
                            errors.append(
                                f"Failed to update rule {suggestion.rule_name}"
                            )
                    else:
                        logger.warning(
                            f"Cannot improve rule '{suggestion.rule_name}' - no similar existing rule found"
                        )
                        errors.append(
                            f"Cannot improve rule '{suggestion.rule_name}' - no similar existing rule found"
                        )
                    ...
                    if folder_name in state.folders:
                        ...
                        folder_id = await state.outlook_client.create_folder(
                            folder_name
                        )
                    ...
                    logger.info(
                        f"Creating new rule: {suggestion.rule_name} -> {folder_name}"
                    )
                    ...
                    logger.warning(
                        f"Unknown rule type '{suggestion.rule_type}' for rule {suggestion.rule_name}"
                    )
                    errors.append(
                        f"Unknown rule type '{suggestion.rule_type}' for rule {suggestion.rule_name}"
                    )
...
def build_graph() -> StateGraph:
    ...
    return builder.compile(
        checkpointer=checkpointer, interrupt_before=["wait_for_approval"]
    )
