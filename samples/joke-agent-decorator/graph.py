...
class CustomFilterAction(GuardrailAction):
    ...
    def handle_validation_result(
        ...
        if isinstance(data, str):
            ...
            print(f"[FILTER][{guardrail_name}] '{self.word_to_filter}' replaced → '{filtered[:80]}'")
            return filtered
...
@pii_detection_guardrail(
    ...
    action=LogAction(severity_level=LoggingSeverityLevel.WARNING, message="Email or phone number detected"),
...
@pii_detection_guardrail(
    ...
    action=BlockAction(title="Person name detection", detail="Person name detected and is not allowed"),
