...
2. When a user message contains a <uip:attachments> tag, call the read_attachments tool with the attachment objects from inside the tag and a query describing what you want to know about the files.
3. Analyze the collected data to provide comprehensive and engaging responses about cinema.
4. Stay updated on current releases, upcoming films, and industry news.
...
graph = create_agent(
    llm,
    tools=[tavily_tool, ReadAttachmentsTool(llm=llm)],
