...
This test suite verifies that guardrails are properly invoked at different scopes:
- Agent-level guardrails (PII detection)
- LLM-level guardrails (Prompt injection)
- Tool-level guardrails (Filter and Block actions, PII detection)
"""
...
class TestAgentWithGuardrails:
    ...
    @pytest.fixture
    def joke_agent_uipath_json(self) -> str:
        """Load the joke agent uipath.json configuration with guardrails."""
        config_path = get_file_path("joke_agent_uipath.json")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                with open("uipath.json", "w", encoding="utf-8") as f:
                    f.write(joke_agent_uipath_json)
                ...
                with patch(
                    "uipath_langchain.chat.models.UiPathChat.ainvoke",
            ...
            try:
                ...
                def mock_evaluate_guardrail(text, guardrail):
                    ...
                    if (
                        ...
                        return MockGuardrailValidationResult(
                        ...
                        return MockGuardrailValidationResult(
                            validation_passed=True, violations=[], reason=""
                        )
                            ...
                            violations=[
                                {
                                    ...
                                    "confidence": 0.95,
                                    "text": "Malicious prompt detected",
                                }
                            ],
