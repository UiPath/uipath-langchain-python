"""Decorator-based joke agent for guardrails parity testing.

This agent implements the same guardrail configuration as parity_agent_middleware.py
using the UiPath decorator API (@pii_detection_guardrail, @prompt_injection_guardrail,
@deterministic_guardrail). Both agents are used in test_guardrails_parity.py to
verify 1:1 behavioral parity between the two guardrail flavors.
...
class CustomFilterAction(GuardrailAction):
    ...
    def _filter(self, text: str) -> str:
        return re.sub(re.escape(self.word_to_filter), self.replacement, text, flags=re.IGNORECASE)
...
# ---------------------------------------------------------------------------
# Tool with stacked guardrail decorators
# (innermost @tool runs first; outer decorators wrap top-down at call time)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# LLM factory with prompt injection + PII guardrails
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Agent factory with AGENT-scope PII guardrail
# ---------------------------------------------------------------------------
