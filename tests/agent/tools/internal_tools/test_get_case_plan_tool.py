"""Tests for get_case_plan_tool.py module."""
...
from uipath_langchain.agent.tools.internal_tools.get_case_plan_tool import (
    PIMS_CASE_PLAN_PATH,
    create_get_case_plan_tool,
)


class TestCreateGetCasePlanTool:
    """Test cases for create_get_case_plan_tool function."""
    ...
    def resource_config(self):
        ...
        properties = AgentInternalGetCasePlanToolProperties(
            tool_type=AgentInternalToolType.GET_CASE_PLAN
        )
        return AgentInternalToolResourceConfig(
            name="get_case_plan",
            description="Fetch the case plan (stages, tasks, rules) by instance id.",
    ...
    @patch(
        "uipath_langchain.agent.tools.internal_tools.get_case_plan_tool.mockable",
        lambda **kwargs: lambda f: f,
    )
    @patch("uipath_langchain.agent.tools.internal_tools.get_case_plan_tool.UiPath")
    ...
    async def test_falls_back_to_runtime_folder_context(
        ...
        mock_uipath = self._mock_uipath_response(
            mock_uipath_class,
            {"case": {"exitConditions": []}, "stages": [{"name": "stage-1"}]},
        )
    ...
    def test_factory_registered_in_handlers(self):
        ...
        assert AgentInternalToolType.GET_CASE_PLAN in _INTERNAL_TOOL_HANDLERS
        assert (
            _INTERNAL_TOOL_HANDLERS[AgentInternalToolType.GET_CASE_PLAN]
            is create_get_case_plan_tool
        )
