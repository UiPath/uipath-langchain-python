"""Tests for get_case_entity_tool.py module."""
...
from uipath_langchain.agent.tools.internal_tools.get_case_entity_tool import (
    PIMS_CASE_ENTITY_PATH,
    create_get_case_entity_tool,
)


class TestCreateGetCaseEntityTool:
    """Test cases for create_get_case_entity_tool function."""
    ...
    def resource_config(self):
        ...
        properties = AgentInternalGetCaseEntityToolProperties(
            tool_type=AgentInternalToolType.GET_CASE_ENTITY
        )
        return AgentInternalToolResourceConfig(
            name="get_case_entity",
            description="Fetch the case entity (business object, documents, comments) by instance id.",
    ...
    @patch(
        "uipath_langchain.agent.tools.internal_tools.get_case_entity_tool.mockable",
        lambda **kwargs: lambda f: f,
    )
    @patch("uipath_langchain.agent.tools.internal_tools.get_case_entity_tool.UiPath")
    ...
    async def test_falls_back_to_runtime_folder_context(
        ...
        mock_uipath = self._mock_uipath_response(
            mock_uipath_class, {"caseSummary": "fallback"}
        )
        ...
        assert result == {"caseSummary": "fallback"}
        args, kwargs = mock_uipath.api_client.request_async.call_args
        assert args == ("GET", PIMS_CASE_ENTITY_PATH.format(instance_id="abc-123"))
    ...
    def test_factory_registered_in_handlers(self):
        ...
        assert AgentInternalToolType.GET_CASE_ENTITY in _INTERNAL_TOOL_HANDLERS
        assert (
            _INTERNAL_TOOL_HANDLERS[AgentInternalToolType.GET_CASE_ENTITY]
            is create_get_case_entity_tool
        )
