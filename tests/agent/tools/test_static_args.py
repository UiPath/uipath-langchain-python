...
class TestApplyStaticArgs:
    """Test cases for apply_static_args decorator."""
    ...
    def mock_async_function(self):
        """Mock async function to test the decorator."""
    ...
    def test_apply_static_args_top_level_simple_fields(self, mock_function):
        """Test applying static args to top level simple fields."""
        static_args = {"field1": "value1", "field2": 42, "field3": True}

        decorated_func = apply_static_args(static_args)(mock_function)

        result = decorated_func(existing_field="existing")
    ...
    def test_apply_static_args_top_level_objects_replace_whole(self, mock_function):
        ...
        result = decorated_func(config={"old_setting": "old_value"})

        expected = {"config": {"new_setting": "new_value", "enabled": True}}
        assert result == expected

    def test_apply_static_args_top_level_arrays_replace_entire(self, mock_function):
        """Test applying static args to top level arrays - should replace entire array."""
        static_args = {"items": ["new_item1", "new_item2"]}

        decorated_func = apply_static_args(static_args)(mock_function)

        result = decorated_func(items=["old_item1", "old_item2", "old_item3"])

        expected = {"items": ["new_item1", "new_item2"]}
        assert result == expected

    def test_apply_static_args_nested_property_in_object_two_levels(
        self, mock_function
    ):
    ...
    def test_apply_static_args_array_element_replace_every_element(self, mock_function):
    ...
    def test_apply_static_args_nested_property_in_array_element(self, mock_function):
    ...
    def test_apply_static_args_with_pydantic_models(self, mock_function):
        ...
        result = decorated_func(model_arg=model_instance)
    ...
    def test_apply_static_args_with_list_of_pydantic_models(self, mock_function):
        ...
        result = decorated_func(models=models)
    ...
    def test_apply_static_args_creates_missing_nested_structure(self, mock_function):
    ...
    def test_apply_static_args_async_function(self, mock_async_function):
        """Test applying static args to an async function."""
        static_args = {"async_field": "async_value"}

        decorated_func = apply_static_args(static_args)(mock_async_function)

        result = asyncio.run(decorated_func(existing="exists"))

        expected = {"existing": "exists", "async_field": "async_value"}
        assert result == expected
