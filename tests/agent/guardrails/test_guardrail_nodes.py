...
class TestLlmGuardrailNodes:
    ...
    async def test_llm_success_pre_and_post(
        self, monkeypatch, execution_stage, expected_name
    ):
        guardrail = types.SimpleNamespace(name="Example")
        ...
        state = types.SimpleNamespace(messages=[HumanMessage("payload")])
        cmd = await node(state)
        assert getattr(cmd, "goto") == "ok"
        assert getattr(cmd, "update") == {"guardrail_validation_result": None}
    ...
    async def test_llm_failure_pre_and_post(
        ...
        assert node_name == expected_name
        state = types.SimpleNamespace(messages=[SystemMessage("payload")])
        cmd = await node(state)
        assert getattr(cmd, "goto") == "nope"
        assert getattr(cmd, "update") == {"guardrail_validation_result": "policy_violation"}
