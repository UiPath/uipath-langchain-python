    ...
    async def _handle(self, request: Request) -> Response:
        ...
        if method == "tools/list":
            return self._result(
                ...
                {
                    "tools": [_tool_schema()],
                    # Required on the 2026-07-28 wire, ignored by older peers.
                    "resultType": "complete",
                    "ttlMs": 0,
                    "cacheScope": "private",
                },
...
def _reset_sse_shutdown_latch() -> None:
    ...
    stops. This testcase hosts one server per leg, so from the second leg
    onward every SSE stream would see the latch already set and end the instant
    it opened -- logging ``ASGI callable returned without completing response``
    and sending the client into a reconnect loop. Clearing the latch (and the
    per-thread watcher bookkeeping) gives every leg the same first-leg
    behaviour.

    Best-effort: these are private names, so a changed internal degrades to the
    noisy-but-working behaviour rather than breaking the matrix.
    """
    try:
...
class SessionHeaderRecorder:
    """Pure ASGI middleware noting every ``mcp-session-id`` a response carries.

    ``2026-07-28`` has no session identity, so a modern leg must show the header
    on *requests* -- carrying the client-minted affinity ID -- and never on a
    response. Recording it at the ASGI layer is the only way to tell a
    server-assigned session from a client-minted one, since both are opaque hex.

    ``starlette.middleware.base.BaseHTTPMiddleware`` is deliberately not used:
    it buffers through an inner task and breaks the server's SSE stream with
    ``ASGI callable returned without completing response``.
    """

    def __init__(self, app: Any) -> None:
        self.app = app
        self.response_session_ids: list[str] = []

    async def __call__(self, scope: Any, receive: Any, send: Any) -> None:
        """Forward one ASGI request, noting any session ID on its response."""
        if scope["type"] != "http":
            ...
            if message["type"] == "http.response.start":
                for key, value in message.get("headers", ()):
                    if key.decode("latin-1").lower() == "mcp-session-id":
                        self.response_session_ids.append(value.decode("latin-1"))
            await send(message)
...
async def serve(app: Any) -> AsyncGenerator[str, None]:
