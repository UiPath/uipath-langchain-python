...
class LegSummary(BaseModel):
    ...
    # What the live MCP session actually settled on. A resumed session that
    # silently downgrades still returns the right session ID, so the ID alone
    # cannot catch it -- this is the field that can.
    negotiated_version: str | None = None
    # Whether the server assigned an ``mcp-session-id`` on any response during
    # this leg. Modern legs must show none: the ID in play is client-minted.
    server_session_issued: bool = False
    disposed: bool = False
...
class AgentsApiResult(BaseModel):
    ...
    modern: LegSummary
    modern_affinity: LegSummary
    modern_affinity_resumed: LegSummary
    debug_state_writes: int
    debug_state_reads: int
    persisted_session_id: str | None = None
    session_resumed: bool = False
    modern_affinity_id_reused: bool = False
...
def _negotiated_version(client: McpClient) -> str | None:
    """Read the protocol version the client's live MCP session settled on.

    ``ClientSession.protocol_version`` is public SDK API, but ``McpClient``
    keeps its session private and exposes no accessor, so this reaches through
    ``_session``. Must be read before ``dispose()`` releases the session.
    """
...
async def _run_leg(
    ...
    Args:
        label: Name this leg is reported under.
        resource: The MCP resource config the caller would carry.
    ...
    if recorder is not None:
        summary.server_session_issued = (
            len(recorder.response_session_ids) > seen_before
        )
...
class _SharedSessionFactory(SessionInfoFactory):
    """Hand every client the same ``SessionInfo``.

    Two clients sharing one store is how a modern affinity ID survives across
    runs -- the playground case, where the store outlives the process.
    """
    ...
    def create_session(self, mcp_server: McpServer) -> SessionInfo:
        """Return the shared SessionInfo regardless of the server."""
...
async def run_agents_api_leg(
    ...
    Args:
        ...
        app: The ASGI app hosting the MCP server and the debug-state route.
        store: The ``DebugStateStore`` mounted on the inner app.
        operands: Operands handed to the remote ``add`` tool.
        recorder: Optional ``servers.SessionHeaderRecorder`` wrapping *app*,
            used to tell a server-assigned session ID from a client-minted one.
    """
    ...
    async with serve(app) as url:
        ...
        with _patched_sdk(url), _agenthub_env(base_url):
            ...
            # 2026-07-28 through the same public client. The era has no session
            # identity, so nothing here may see a server-assigned one.
            modern = await _run_leg(
                "modern",
                _dynamic_resource(),
                session_info_factory=None,
                terminate_on_close=True,
            ...
            # Two modern clients sharing one store: the affinity ID the first
            # mints must be the one the second sends, so a gateway keeps routing
            # both runs to the same warm instance.
            shared = SessionInfo()
            shared_factory = _SharedSessionFactory(shared)
            modern_affinity = await _run_leg(
                "modern-affinity",
                _cached_resource(),
                session_info_factory=shared_factory,
                terminate_on_close=False,
                operands=operands,
                protocol_mode="modern",
                recorder=recorder,
            )
            modern_affinity_resumed = await _run_leg(
                "modern-affinity-resumed",
                _cached_resource(),
                session_info_factory=shared_factory,
                terminate_on_close=False,
                operands=operands,
    ...
    return AgentsApiResult(
        ...
        modern=modern,
        modern_affinity=modern_affinity,
        modern_affinity_resumed=modern_affinity_resumed,
        ...
        modern_affinity_id_reused=bool(
            modern_affinity.session_id
            and modern_affinity.session_id == modern_affinity_resumed.session_id
        ),
    )
