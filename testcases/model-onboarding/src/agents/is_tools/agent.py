...
Agent" from the low-code IsToolsAgent (alpha, solution 925fc5db-…, project
0959f23b-… — download via ``uip agent file get``); this module is that file
with exactly one structural change: the module-level hardcoded
``llm`` is replaced by ``build_tools()`` / ``build_graph(llm)`` so the
onboarding testcase can inject the model built from ``model_spec`` (model +
API flavor configurable).

The tools are real IS Activity tools — Slack *Send Message to Channel* and
Outlook 365 *Send Email*. The testcase ``run`` is **call-only**: it binds the
generated tools to the model under test and asserts the model produces a
well-formed tool call. The tools are never executed, so no connections are
needed and nothing is actually sent. ``build_graph`` remains for full
executed runs (requires real connection ids; performs real actions).
"""
...
def build_tools(connections=None):
    """Build the generated IS Activity tools (Studio Web coded copy, verbatim).

    ``connections`` optionally supplies the IS connection ids the maker would
    pick in the designer. For the call-only onboarding test no connections are
    needed — the tools are bound to the model but never executed.
    """
...
def build_graph(llm, connections=None):
    """Build the generated agent graph (full ReAct run — executes the tools).

    Mirrors the Studio Web coded copy's module body; ``llm`` replaces the
    hardcoded ``get_chat_model(model='gpt-5.4', ...)``. Only use with real
    connection ids — a run performs real communication actions.
    """
    tools = build_tools(connections)

    # Create agent graph
    return create_agent(model=llm, messages=create_messages, tools=tools, input_schema=AgentInput, output_schema=AgentOutput)


# Required args the model must fill for a call to count as well-formed.
# Keys are the BOUND tool names (create_integration_tool underscores them).
_REQUIRED_ARGS = {
    "Send_Message_to_Channel": lambda a: a.get("channel") and a.get("messageToSend"),
    "Send_Email": lambda a: (a.get("message") or {}).get("toRecipients"),
}


async def run(model, prompt: str) -> str:
    """Testcase adapter: call-only — assert the model produces the tool call.

    Binds the generated Activity tools to the model under test, sends the
    exact production messages (``create_messages``), and requires at least one
    well-formed tool call back. The tools are NEVER executed, so no IS
    connections are needed and nothing is actually sent.

    Returns ``"✓ (names)"`` or ``"✗ ..."`` per the onboarding cell contract.
    """
    tools = build_tools()
    messages = list(create_messages(AgentInput(prompt=prompt)))
    ...
    for call in calls:
        ...
        check = _REQUIRED_ARGS.get(call["name"])
        if check and not check(call.get("args") or {}):
            return f"✗ tool call '{call['name']}' missing required args"
        well_formed.append(call["name"])
    return f"✓ ({', '.join(well_formed)})"
