...
class UiPathChatMessagesMapper:
    ...
    def map_ai_message_chunk_to_events(
        ...
        if message.chunk_position == "last":
            if (
                ...
                events.extend(self.map_current_message_to_start_tool_call_events())
            else:
                events.append(self.map_to_message_end_event(message.id))

        return events

    def map_current_message_to_start_tool_call_events(self):
        events: list[UiPathConversationMessageEvent] = []
        if (
    ...
    async def get_message_id_for_tool_call(
        ...
        # Look through messages in reverse order keeping track of the ToolMessage id's. When we find the last AIMessage,
        # look for the provided tool_call_id in the message's ids. We expect it to be found there, and that gives us the
        # corresponding message id. We also check to see if all the message's tool calls have a corresponding
        # ToolMessage, which indicates that all tools have finished executing and the endMessage event can be sent.

        tool_call_ids_seen = set[str]()
        for message in reversed(messages):
            if isinstance(message, ToolMessage):
                tool_call_ids_seen.add(message.tool_call_id)
            elif isinstance(message, AIMessage):
                if message.tool_calls is not None:
                    message_tool_call_ids = set(
                        map(lambda tool_call: tool_call["id"], message.tool_calls)
                    )
                    if tool_call_id in message_tool_call_ids:
                        pending_tool_call_ids = message_tool_call_ids.difference(
                            tool_call_ids_seen
                        )
                        is_last_tool_call = (
                            len(pending_tool_call_ids) == 1
                            and tool_call_id in pending_tool_call_ids
                        )
                        if not is_last_tool_call and len(pending_tool_call_ids) == 1:
                            logger.warning(
                                f"only pending tool call ({pending_tool_call_ids}) is not tool call {tool_call_id}"
                            )
                        return message.id, is_last_tool_call
                else:
                    logger.warning(
                        f"tool call id {tool_call_id} not found in most recent AI message {message.id}"
                    )
                break
        return None, False
