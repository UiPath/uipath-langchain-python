...
def _wrap_tool_with_guardrail(
    ...
    tool.__class__ = _GuardedTool  # type: ignore[assignment]
    return tool
...
def _wrap_llm_with_guardrail(
    ...
    llm.__class__ = _GuardedLLM  # type: ignore[assignment]
    return llm
...
class LangChainGuardrailAdapter:
    ...
    def detect_scope(self, target: Any) -> GuardrailScope | None:
        """Return the guardrail scope for a LangChain/LangGraph object."""
        if isinstance(target, BaseTool):
            return GuardrailScope.TOOL
        if isinstance(target, BaseChatModel):
            return GuardrailScope.LLM
        if isinstance(target, (StateGraph, CompiledStateGraph)):
