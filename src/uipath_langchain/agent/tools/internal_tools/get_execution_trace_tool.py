...
_GET_EXECUTION_TRACE_ERRORS: dict[
    ...
    (401, None): (
        "Unauthorized when fetching execution trace for tool '{tool}': {message}",
        UiPathErrorCategory.SYSTEM,
    ),
    (403, None): (
        "Forbidden when fetching execution trace for tool '{tool}': {message}",
        UiPathErrorCategory.USER,
    ),
}
...
def create_get_execution_trace_tool(
    ...
    Calls the PIMs ``element-executions/case-instances`` (v2) endpoint with the
    agent's UiPath credentials and folder context. Returns the audit trail of
    the case instance — which rules fired, which agent invocations occurred,
    what was decided, and in what order — used by the agent to detect patterns,
    avoid duplicate actions, and reason about why the case is in its current
    state.
    ...
    async def tool_fn(**kwargs: Any) -> Any:
        ...
        client = UiPath()
        url = PIMS_EXECUTION_TRACE_PATH.format(instance_id=instance_id)
        ...
        try:
            ...
            raise_for_enriched(
                e, _GET_EXECUTION_TRACE_ERRORS, title=tool_name, tool=tool_name
            )
            raise
