...
_GET_CASE_PLAN_ERRORS: dict[
    ...
    (401, None): (
        "Unauthorized when fetching case plan for tool '{tool}': {message}",
        UiPathErrorCategory.SYSTEM,
    ),
    (403, None): (
        "Forbidden when fetching case plan for tool '{tool}': {message}",
        UiPathErrorCategory.USER,
    ),
}
...
def create_get_case_plan_tool(
    ...
    Calls the PIMs ``case-rules`` endpoint with the agent's UiPath credentials
    and folder context. Returns the full case plan: every stage and task with
    its entry/exit rules, ``isRequired``, and ``shouldRunOnlyOnce``.
    ...
    async def tool_fn(**kwargs: Any) -> Any:
        ...
        client = UiPath()
        url = PIMS_CASE_PLAN_PATH.format(instance_id=instance_id)
        ...
        try:
            ...
            raise_for_enriched(
                e, _GET_CASE_PLAN_ERRORS, title=tool_name, tool=tool_name
            )
            raise
