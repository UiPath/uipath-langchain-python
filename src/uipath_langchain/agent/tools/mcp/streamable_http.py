...
Adapted from mcp.client.streamable_http (MCP Python SDK 1.26) to support
SessionInfo for external session ID tracking.
...
class StreamableHTTPTransport:
    ...
    @overload
    def __init__(
        self, url: str, *, session_info: SessionInfo | None = None
    ) -> None: ...
    ...
    async def _maybe_extract_session_id_from_response(
        self,
    ...
    def _maybe_extract_protocol_version_from_message(
        ...
        if (
            isinstance(message.root, JSONRPCResponse) and message.root.result
        ):  # pragma: no branch
            try:
    ...
    async def _handle_resumption_request(self, ctx: RequestContext) -> None:
        ...
        if isinstance(
            ctx.session_message.message.root, JSONRPCRequest
        ):  # pragma: no branch
    ...
    async def _handle_post_request(self, ctx: RequestContext) -> None:
        ...
        async with ctx.client.stream(
            ...
            if response.status_code >= 400:
                ...
                logger.error(
                    f"HTTP {response.status_code} from POST {self.url}: {body.decode(errors='replace')}"
                )
