...
class GraphResolverContext:
    """
    Async context manager wrapping GraphResolver.
    Returns a callable that can be passed directly as AsyncResolver to LangGraphRuntime.
    """

    def __init__(self, entrypoint: Optional[str] = None) -> None:
        self._resolver = GraphResolver(entrypoint)
        self._graph: Optional[StateGraph[Any, Any, Any]] = None

    async def __aenter__(self) -> AsyncResolver:
        # Lazily resolve the graph and return a callable
        async def resolver_callable() -> StateGraph[Any, Any, Any]:
            if self._graph is None:
                self._graph = await self._resolver()
            return self._graph
